...
namespace Orso.Arpa.Application.Tests.ValidationTests
{
    ...
    public class SetMyProjectParticipationDtoValidatorTests
    {
        ...
        {
            _bodyValidator.ShouldHaveValidationErrorFor(command => command.StatusId, Guid.Empty);
        }
        ...
        {
            _bodyValidator.ShouldHaveValidationErrorFor(command => command.Comment, new string('#', 501));
        }

        [Test]
        public void Should_Not_Have_Validation_Error_If_Valid_Comment_Is_Supplied()
        {
            _bodyValidator.ShouldNotHaveValidationErrorFor(command => command.Comment, new string('#', 500));
        }
    }
}

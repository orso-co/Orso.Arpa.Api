...
public class AppointmentsControllerTests : IntegrationTestBase
{
        ...
        IList<AppointmentListDto> expectedDtos)
    {
        ...
        IEnumerable<AppointmentListDto> result =
            await DeserializeResponseMessageAsync<IEnumerable<AppointmentListDto>>(responseMessage);
    ...
    public async Task Should_Get_By_Id_With_Participations(AppointmentDto expectedDto)
    {
        ...
        AppointmentDto result =
            await DeserializeResponseMessageAsync<AppointmentDto>(responseMessage);
    ...
    public async Task Should_Not_Modify_If_Not_Existing_Id_Is_Supplied()
    {
        ...
        ValidationProblemDetails errorMessage =
            await DeserializeResponseMessageAsync<ValidationProblemDetails>(responseMessage);
        _ = errorMessage.Title.Should().Be("Resource not found.");
        _ = errorMessage.Status.Should().Be(404);
        _ = errorMessage.Errors.Should()
            .BeEquivalentTo(
                new Dictionary<string, string[]> { { "Id", s_appointmentNotFoundMessage } });
    }

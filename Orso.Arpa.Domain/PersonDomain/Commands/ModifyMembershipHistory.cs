...
{
    public static class ModifyMembershipHistory
    {
        public class Command : IModifyCommand<MembershipHistory>
        {
            ...
            public Validator(IArpaContext arpaContext)
            {
                RuleFor(c => c.Id)
                    .MustAsync(async (command, id, cancellation) => await arpaContext
                        .EntityExistsAsync<MembershipHistory>(h => h.Id == id && h.PersonMembershipId == command.PersonMembershipId, cancellation))
                    .WithMessage("Membership history entry could not be found")
                    .WithErrorCode("404");

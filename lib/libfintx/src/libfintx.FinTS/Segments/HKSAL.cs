...
namespace libfintx.FinTS
{
    public static class HKSAL
    {
        ...
        public static async Task<String> Init_HKSAL(FinTsClient client)
        {
            ...
            // Only add HKTAN when using PIN:2 (two-step with TAN method).
            // In PIN:1 mode (HIRMS is null), adding HKTAN causes
            // "Falsche Segmentzusammenstellung:HKTAN:segvers=7:option=999".
            if (client.BPD.IsTANRequired("HKSAL") && !string.IsNullOrEmpty(client.HIRMS))
            {
